...
local EntityNotify = {
    ["GloombatSwarm"] = "Gloombats 将在下一扇门出现！"
}
...
local Tabs = {
    ...
    Exploits = Window:AddTab("第二页"),
    Visuals = Window:AddTab("第三页"),
...
function Script.Functions.DoorESP(room)
    ...
    if door then
        ...
        local doorState = opened and " [打开]" or (locked and " [上锁]" or "")
        ...
        door:GetAttributeChangedSignal("Opened"):Connect(function()
            doorEsp.Text = string.format("门 %s [打开]", doorNumber)
        end)
    end
end 
...
function Script.Functions.ObjectiveESP(child)
    ...
    if child.Name == "TimerLever" then
        Script.Functions.ESP({
            Type = "Objective",
            Object = child,
            Text = string.format("直接拉杆 [+%s]", child.TakeTimer.TextLabel.Text),
...
function Script.Functions.GoldESP(gold)
    Script.Functions.ESP({
        Type = "Gold",
        Object = gold,
        Text = string.format("黄金 [%s]", gold:GetAttribute("GoldValue")),
...
function Script.Functions.GetAllPromptsWithCondition(condition)
    assert(typeof(condition) == "功能", "需要一个函数作为条件参数，但得到了" .. typeof(condition))
    ...
    for _, prompt in pairs(PromptTable.GamePrompts) do
        ...
        assert(typeof(returnData) == "boolean", "期望条件函数返回布尔值")
        ...
        if mainGameSrc.stopcam and child.Name == "ElevatorBreaker" and Toggles.AutoBreakerSolver.Value then
            ...
            if not child:GetAttribute("Solving") then
                ...
                if code and code:FindFirstChild("Frame") then
                    ...
                    autoConnections["密码"] = code:GetPropertyChangedSignal("Text"):Connect(function()
                        task.wait(0.1)
    ...
    Script.Connections[room.Name .. "DescendantAdded"] = room.DescendantAdded:Connect(function(child)
        ...
        task.spawn(function()
            if child.Name == "TriggerEventCollision" and Toggles.DeleteSeek.Value and character then
                Script.Functions.Alert("删除Seek...", child:FindFirstChildOfClass("BasePart"))
                ...
                Script.Functions.Alert("成功删除Seek！好好玩", 5)
            end
        end)
    end)
end
...
local ReachGroupBox = Tabs.Main:AddLeftGroupbox("长臂猿") do
...
local MiscGroupBox = Tabs.Main:AddRightGroupbox("退出") do
...
local AntiEntityGroupBox = Tabs.Exploits:AddLeftGroupbox("取消怪物的一些功能") do
    ...
    AntiEntityGroupBox:AddToggle("AntiDupe", {
        Text = "无效" .. (isBackdoor and "真空假门" or "假门"),
        Default = false
    })

    AntiEntityGroupBox:AddToggle("AntiEyes", {
        Text = "无效" .. (isBackdoor and "Lookman" or "Eyes"),
        Default = false
    })
...
local TrollingGroupBox = Tabs.Exploits:AddLeftGroupbox("Fun") do
...
local BypassGroupBox = Tabs.Exploits:AddRightGroupbox("绕过") do
    BypassGroupBox:AddDropdown("速度绕过", {
        AllowNull = false,
...
local SelfGroupBox = Tabs.Visuals:AddRightGroupbox("自己") do
    ...
    SelfGroupBox:AddToggle("TranslucentHidingSpot", {
        Text = "透明 " .. HidingPlaceName[floor.Value],
        Default = false
    })
...
task.spawn(function()
    if isHotel then
        local Hotel_AntiEntityGroupBox = Tabs.Floor:AddLeftGroupbox("移除类") do
    ...
    elseif isMines then
        ...
        local Mines_AutomationGroupBox = Tabs.Floor:AddRightGroupbox("Automation") do
            Mines_AutomationGroupBox:AddButton({
                Text = "到达200门200",
                Func = function()
                    if latestRoom.Value < 99 then Script.Functions.Alert("你还没有到达200号门...") end
        ...
        Toggles.TheMinesAnticheatBypass:OnChanged(function(value)
            if value then
                ...
                if Library.IsMobile then
                    Script.Functions.Alert("要绕过防热，你必须与梯子互动，梯子ESP已启用", progressPart)
                else
                    Script.Functions.Alert("要绕过防热，你必须与梯子互动，梯子ESP已启用", progressPart)
                end
    ...
    elseif isRooms then
        ...
        Toggles.AntiA90:OnChanged(function(value)
            if Toggles.AutoRooms.Value and not value then
                Script.Functions.Alert("自动Rooms需要防A-90才能工作！", 5)
    ...
    elseif isFools then
        ...
        Toggles.FigureGodmodeFools:OnChanged(function(value)
            if value and not Toggles.GodmodeNoclipBypassFools.Value then Toggles.GodmodeNoclipBypassFools:SetValue(true); Script.Functions.Alert("使用数字Godmode需要Godmode/Noclip旁路") end
...
Toggles.FakeRevive:OnChanged(function(value)
    if value and alive and character and not fakeReviveEnabled then
        if latestRoom and latestRoom.Value == 0 then
            Script.Functions.Alert("你必须打开下一扇门才能使用假复活")
            repeat task.wait() until latestRoom.Value > 0
        end

        Script.Functions.Alert("请想办法死去或者等待20秒左右\n让假复活起作用", 20)
        ...
        if alive and not Toggles.FakeRevive.Value then
            ...
            Script.Functions.Alert("假复活已被禁用，无法杀死玩家")
		...
		local function usePreviewCharacter(doStepped)
			...
			if doStepped ~= false then
				Library:Notify("其他人看不到您，因为您已经失去了角色的网络所有权", 5);
        ...
        fakeReviveConnections["mainStepped"] = RunService.RenderStepped:Connect(function()
			...
			if not character:FindFirstChild("HumanoidRootPart") then
				Library:Notify("你已经完全失去了你的角色的网络所有权，这导致了打破假死", 5);
		...
		Script.Functions.Alert("假死现在开启了，好好玩！", 5)
    end
end)
            ...
            task.spawn(function()
                ...
                if child:IsDescendantOf(workspace) then
                    ...
                    if Toggles.NotifyEntity.Value then
                        Script.Functions.Alert(entityName .. " 已经生成!")
                    end
                end
            end)
